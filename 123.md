...
>默认使用来自 `luasnip.config.snip_env` 的名称,但也可以在 `setup` 中设置 `snip_env` 自定义。
...
在 Snippets 一节有更详细解释，大意是它会创建一个代码段，包含节点中指定的各个节点。
...
- `key`  
可以引用节点。对 [Key Indexer](#key-indexer-zh-luasnip-key-indexer) 或在运行时查找节点（见 Snippets-API）有用，例如在 `dynamicNode` 里。
这个 key 在 snippet 的整个生命周期内不需要是唯一的，但在任意时刻，一个 snippet 不能有重复 key。
这意味着，可以从 `dynamicNode` 返回带 key 的节点。
因为，即使它们被生成多次，只要不在同一时刻，就不会有冲突。
- `node_callbacks`：为节点定义 events-callbacks（见 event）。  
接受一个表，将事件（例如 `event.enter`）映射到 callback（本质上和转递给 `s` 的 `callbacks` 一样）。
...
（见 `api-indexing`，这里使用 extmarks 索引）。
  - `opts`：`table|nill`  
    - `raw`：`bool`，默认 `true`。  
